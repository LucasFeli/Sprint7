  ...
  return (
    <>
      ...
      {data.results.map((ships) => {
        ...
        return (
          <div key={ships.name} className="card card-side bg-base-100 shadow-xl">
          <figure><img src={imageUrl} alt={ships.name} style={{ width: '300px', height: '200px' }}
                    onError={(e) => {
          ...
          <div className="card-body">
            ...
            <div className="card-actions justify-end">
              <button className="btn btn-primary"><Link to={`/starships/${id}`}>More Info</Link></button>
            </div>
          </div>
        </div>
        );
